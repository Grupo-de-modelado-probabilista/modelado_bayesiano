...
chains = [[(-2.5, 1.15), (-0.9, 1.2), (0.5, 1.6), (2.1, 1.3)],
...
def update(idx):
    ...
    for jdx, chain in enumerate(chains[idx]):
        ...
        az.plot_kde(sample, ax=axes[0], plot_kwargs={"color":f"C{jdx}", "lw":2, "label":f"chain {jdx}"})
        samples.append(sample)

    within_variance = np.mean(np.var(samples, 1))
    az.plot_kde(np.stack(samples), ax=axes[1], plot_kwargs={"color":"k", "lw":3, "label":"varianza total"})
    az.plot_kde(stats.norm(np.mean(samples), within_variance**0.5).rvs(2000),
                ax=axes[1], plot_kwargs={"color":"0.5", "ls":"--", "lw":3, "label":"varianza interna"})
    
    r_hat = az.rhat({"a":samples})
    axes[1].set_title(f"$\hat R$={r_hat['a'].item():.2f}\nvarianza total={np.var(samples):.2f}\nvarianza interna{within_variance:.2f}")
